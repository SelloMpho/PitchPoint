...
export default function StartupDetail() {
    ...
    const mockStartup: Startup = {
      ...
      teamMembers: [
        ...
        },
        {
          ...
          linkedin: 'https://linkedin.com/in/michaelndlovu'
        }
      ],
      traction: {
        revenue: 'R150,000 in 2023 Q1',
        customers: '25 residential installations completed',
        growth: '200% YoY growth in installations',
        partnerships: 'Strategic partnership with Cape Town Green Initiative'
      },
      owner: {
        _id: 'user123',
        firstName: 'John',
  ...
  const handleBookmark = () => {
    ...
    alert(isBookmarked ? 'Startup removed from bookmarks' : 'Startup added to bookmarks');
  };
  ...
  const handleContactClick = () => {
    ...
    if (userRole !== 'investor') {
      alert('Only investors can contact startups');
      return;
    }
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <div className="flex-grow flex items-center justify-center py-20">
          <div className="glass-effect rounded-2xl p-8 text-center border border-slate-700/50 backdrop-blur-sm max-w-md">
            ...
            <p className="text-slate-300 text-lg mb-4">{error || 'Startup not found'}</p>
            <Link 
              href="/startups"
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
              ...
              <div className="bg-slate-800/50 px-8 pt-8 pb-6">
                <div className="sm:flex sm:items-start">
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 className="text-2xl leading-6 font-bold text-white mb-6">Contact {startup.name}</h3>
                    <form onSubmit={(e) => { e.preventDefault(); handleContactSuccess(); }}>
                      ...
                      <div className="mb-6">
                        ...
                        <textarea
                          ...
                          placeholder="Please share details about your investment interest and how you can add value to this startup..."
                        ></textarea>
                      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-6">
          <Link 
            href="/startups"
            className="text-indigo-400 hover:text-indigo-300 flex items-center transition-colors duration-200 group"
          >
        ...
        <div className="glass-effect rounded-2xl overflow-hidden border border-slate-700/50 backdrop-blur-sm">
          {/* Header Section */}
          <div className="bg-gradient-to-r from-indigo-900/30 to-purple-900/30 p-6 md:p-8 flex flex-col lg:flex-row justify-between items-start lg:items-center border-b border-slate-700/50">
            <div className="flex items-center mb-6 lg:mb-0">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white mb-2">{startup.name}</h1>
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
              {isLoggedIn && userRole === 'investor' && (
                <button 
                  ...
                  className="flex-1 bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-3 rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex items-center justify-center"
                >
              ...
              {isLoggedIn && userRole === 'investor' && (
                <button 
                  ...
                  className="flex-1 bg-slate-800/50 text-white px-6 py-3 rounded-xl hover:bg-slate-700/50 transition-all duration-200 font-semibold border border-slate-600 flex items-center justify-center"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              <div className="lg:col-span-2 space-y-6 md:space-y-8">
                <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                  <h2 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">About {startup.name}</h2>
                ...
                <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                    {startup.teamMembers.map((member, index) => (
                      <div key={index} className="bg-slate-800/50 rounded-xl p-4 md:p-6 border border-slate-700/50 hover:bg-slate-700/50 transition-colors duration-200">
                        <div className="flex items-center mb-3 md:mb-4">
                          <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-full flex items-center justify-center mr-3 md:mr-4">
                          ...
                          <div>
                            ...
                            <p className="text-indigo-400 text-sm md:text-base">{member.role}</p>
                          </div>
                        </div>
                        <p className="text-slate-300 text-sm mb-3 md:mb-4">{member.bio}</p>
                        {member.linkedin && (
                          <a 
                ...
                <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                  <h2 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">Traction & Milestones</h2>
                  <div className="bg-slate-800/50 rounded-xl p-4 md:p-6 border border-slate-700/50">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                      {startup.traction.revenue && (
                        <div>
                          <h3 className="text-slate-400 text-xs md:text-sm mb-1">REVENUE</h3>
                          <p className="text-white font-bold text-sm md:text-base">{startup.traction.revenue}</p>
                        </div>
                      )}
                      
                      {startup.traction.customers && (
                        <div>
                          <h3 className="text-slate-400 text-xs md:text-sm mb-1">CUSTOMERS</h3>
                          <p className="text-white font-bold text-sm md:text-base">{startup.traction.customers}</p>
                        </div>
                      )}
                      
                      {startup.traction.growth && (
                        <div>
                          <h3 className="text-slate-400 text-xs md:text-sm mb-1">GROWTH</h3>
                          <p className="text-white font-bold text-sm md:text-base">{startup.traction.growth}</p>
                        </div>
                      )}
                      
                      {startup.traction.partnerships && (
                        <div>
                          <h3 className="text-slate-400 text-xs md:text-sm mb-1">PARTNERSHIPS</h3>
                          <p className="text-white font-bold text-sm md:text-base">{startup.traction.partnerships}</p>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {startup.useOfFunds && (
                  <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                    <h2 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">Use of Funds</h2>
                    <div className="bg-slate-800/50 rounded-xl p-4 md:p-6 border border-slate-700/50">
                      <p className="text-slate-300 text-sm md:text-base">{startup.useOfFunds}</p>
                    </div>
                  </section>
                )}
              </div>
              
              {/* Right Column - Investment Details & Links */}
              <div className="space-y-6">
                <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                  <h2 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">Investment Details</h2>
                  
                  <div className="space-y-4 md:space-y-6">
                    <div>
                      <h3 className="text-slate-400 text-xs md:text-sm mb-2">FUNDING STAGE</h3>
                      <p className="text-white font-bold text-sm md:text-base">{startup.fundingStage}</p>
                    </div>
                    
                    <div>
                      <h3 className="text-slate-400 text-xs md:text-sm mb-2">FUNDING ASK</h3>
                      <p className="text-white font-bold text-sm md:text-base">R{startup.fundingAmount?.toLocaleString()}</p>
                    </div>
                    
                    <div>
                      <h3 className="text-slate-400 text-xs md:text-sm mb-2">EQUITY OFFERED</h3>
                      <p className="text-white font-bold text-sm md:text-base">{startup.equityOffered}%</p>
                    </div>
                    
                    <div>
                      <h3 className="text-slate-400 text-xs md:text-sm mb-2">FOUNDED</h3>
                      <p className="text-white font-bold text-sm md:text-base">{startup.foundingYear}</p>
                    </div>
                  </div>
                </section>
                
                <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                  <h2 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">Resources</h2>
                  ...
                  {startup.pitchDeck && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="flex items-center text-indigo-400 hover:text-indigo-300 mb-4 transition-colors duration-200 group"
                    >
                      ...
                      <span className="text-sm md:text-base">View Pitch Deck</span>
                    </a>
                  )}
                  ...
                  {startup.pitchVideo && (
                    ...
                    >
                      ...
                      <span className="text-sm md:text-base">Watch Pitch Video</span>
                    </a>
                  )}
                </section>
                ...
                <section className="glass-effect rounded-2xl p-6 md:p-8 border border-slate-700/50 backdrop-blur-sm">
                  ...
                  {startup.socialMedia && Object.entries(startup.socialMedia).map(([platform, url]) => (
                    <a 
                      key={platform}
                      href={url as string} 
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="flex items-center text-indigo-400 hover:text-indigo-300 mb-4 last:mb-0 transition-colors duration-200 group"
                    >
                      <div className="w-8 h-8 md:w-10 md:h-10 bg-slate-800/50 rounded-full flex items-center justify-center mr-3 md:mr-4 group-hover:bg-indigo-600/20 transition-colors duration-200">
                        {platform === 'linkedin' && (
                      ...
                      <span className="text-sm md:text-base">{platform.charAt(0).toUpperCase() + platform.slice(1)}</span>
                    </a>
                  ))}
                </section>
              </div>
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              ...
              <Link href="/investors" className="text-slate-300 hover:text-white px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200">Investors</Link>
              <Link href="/startups" className="text-white px-3 py-2 rounded-md text-sm font-medium bg-slate-700/50">Startups</Link>
